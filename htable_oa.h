...
static int tname ## _insert(tname * self, K key, V el)\
{\
...
static const V * tname ## _find(tname * self, const K key)\
{\
	size_t h = self->hash_func(self->size, (const K*)&key);\
	size_t i;\
	if(self->cmp_func)\
