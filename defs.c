...
double tanh(double x) { return 2 * sigmoid(x) - 1; }
...
double neg_log_likelihood_ns_loss(vec y, vec _y, void* _args)
{
...
double d_neg_log_likelihood_ns_loss(vec y, vec _y, size_t i, void* _args)
{
	struct neg_log_likelihood_ns_loss_args* args = _args;
	if(args->neg_idx[0] == i)
		return _y.data[i] - 1;
